...
class ProxyControl:
    ...
    def detect_proxy(self):
        with self.mysql() as cursor:
            try:
                cursor.execute("SELECT * from proxysbing where score > 7 and (last_use is null or last_use < date_sub(now(), interval 2 hour))")
                print "---------------------------detect_proxy---------------------------"
    ...
    def amazon_check(self, proxies):
        try:
            ...
            r = requests.get(url='https://cn.bing.com', headers=headers, proxies=proxies)
            ...
            print ("------------check to bing error {}".format(e))
            speed = -1
            protocol = -1
            types = -1
    ...
    def update_proxy(self, proxy, cursor):
        update_sql = "UPDATE proxysbing set score = '%(score)s',speed = '%(speed)s'  where ip = '%(ip)s' and port = '%(port)s'" % proxy
        try:
            row_count = cursor.execute(update_sql)
            print("UPDATE proxysbing {} success:", row_count)
        except Exception as e:
            print("UPDATE proxysbing errors:{}".format(e), proxy)
